...
use warp::{filters::BoxedFilter, Reply, reply::{WithStatus, Json}, Filter, hyper::StatusCode};

use crate::{environment::{HCAPTCHA_SECRET, JWT_SECRET, SALT}, utilities::{vec_to_array, generate_id}, database::user::User};
...
pub async fn handle(register: Register) -> Result<WithStatus<Json>, warp::Rejection> {
    ...
    let result = client
        ...
        .query(&[("response", register.captcha_token), ("secret", HCAPTCHA_SECRET.to_string())])
        .send()
        .await;
    if let Ok(r) = result {
        if r.status() == reqwest::StatusCode::OK {
            let text = r.text().await.expect("Unexpected error: failed to read response");
            let response: HCaptchaResponse = serde_json::from_str(&text).expect("Unexpected error: failed to convert response into JSON");
            if response.success {
                let salt_bytes =
                    decode(&*SALT).expect("Unexpected error: failed to convert salt to bytes");
                let hashed =
                    hash_with_salt(register.email.clone(), DEFAULT_COST, vec_to_array::<u8, 16>(salt_bytes))
                        .expect("Unexpected error: failed to hash");
                let password_hash = hash(register.password, DEFAULT_COST).expect("Unexpected error: failed to hash");
